...
pub unsafe fn fire_fly_main(weapon: &mut L2CFighterBase) -> L2CValue {
    ...
    let rot = Vector3f{x: angle, y: 0.0, z: 0.0};
    PostureModule::set_rot(module_accessor,&rot,0); //maybe try multiplying cos of angle with speed once?
    weapon.global_table[0x15].assign(&L2CValue::Ptr(timer as *const () as _));
...
pub unsafe fn fire2_fly_main(weapon: &mut L2CFighterBase) -> L2CValue {
    let lua_state = weapon.lua_state_agent;
    let module_accessor = smash::app::sv_system::battle_object_module_accessor(lua_state);
    let param_type = WorkModule::get_int64(module_accessor,*WEAPON_JACK_FIRE_INSTANCE_WORK_ID_INT_PARAM_ID);
    ...
    || WorkModule::get_int(owner_owner_module_accessor,FIGHTER_JACK_INSTANCE_WORK_ID_INT_PERSONA_KIND) == PERSONA_KIND_YOSHITSUNE {
        angle = -5.0;
    }
    if WorkModule::get_int(owner_owner_module_accessor,FIGHTER_JACK_INSTANCE_WORK_ID_INT_PERSONA_KIND) == PERSONA_KIND_FORTUNA {
        speed = 6.0;
    }
    if WorkModule::get_int(owner_owner_module_accessor,FIGHTER_JACK_INSTANCE_WORK_ID_INT_PERSONA_KIND) == PERSONA_KIND_ORPHEOUS
    || WorkModule::get_int(owner_owner_module_accessor,FIGHTER_JACK_INSTANCE_WORK_ID_INT_PERSONA_KIND) == PERSONA_KIND_MESSIAH {
        WorkModule::set_int(module_accessor,34,*WEAPON_INSTANCE_WORK_ID_INT_LIFE);
        WorkModule::set_int(module_accessor,34,*WEAPON_INSTANCE_WORK_ID_INT_INIT_LIFE);
    }
    if WorkModule::get_int(owner_owner_module_accessor,FIGHTER_JACK_INSTANCE_WORK_ID_INT_PERSONA_KIND) == PERSONA_KIND_KAGUYA
    || WorkModule::get_int(owner_owner_module_accessor,FIGHTER_JACK_INSTANCE_WORK_ID_INT_PERSONA_KIND) == PERSONA_KIND_CENDRILLON
    || WorkModule::get_int(owner_owner_module_accessor,FIGHTER_JACK_INSTANCE_WORK_ID_INT_PERSONA_KIND) == PERSONA_KIND_VANADIS
    || WorkModule::get_int(owner_owner_module_accessor,FIGHTER_JACK_INSTANCE_WORK_ID_INT_PERSONA_KIND) == PERSONA_KIND_ELLA {
        WorkModule::set_int(module_accessor,55,*WEAPON_INSTANCE_WORK_ID_INT_LIFE);
        WorkModule::set_int(module_accessor,55,*WEAPON_INSTANCE_WORK_ID_INT_INIT_LIFE);
    }
    if WorkModule::get_int(owner_owner_module_accessor,FIGHTER_JACK_INSTANCE_WORK_ID_INT_PERSONA_KIND) == PERSONA_KIND_OKUNINUSHI {
        WorkModule::set_int(module_accessor,5,*WEAPON_INSTANCE_WORK_ID_INT_LIFE);
        WorkModule::set_int(module_accessor,5,*WEAPON_INSTANCE_WORK_ID_INT_INIT_LIFE);
    }
